...
- Getting a first IAP into a submission — three paths, in preference order (verify at submission time, M17, when a build exists):
  1. **Raw `reviewSubmissionItems` API** (curl + API key) — Apple documents `inAppPurchaseV2` as a valid review-submission item type, so `POST /v1/reviewSubmissionItems` with an `inAppPurchaseV2` relationship should add the IAP to a review submission with NO browser. Untested on our account as of 2026-05-24 — confirm when first used.
  2. **Chrome MCP** against the signed-in Chrome (version page → "Select In-App Purchases or Subscriptions" → check → Done) — the *verified* path (Linewright). Needs Chrome actually logged into ASC (a fresh ASC nav redirecting to `login?...authResult=FAILED` means it isn't, even if an old `/apps` tab looks signed in).
  3. ⚠️ The `asc` CLI does **NOT** wrap this: `asc review items-add --item-type` (asc 1.5.1) omits `inAppPurchaseV2` (only versions/events/experiments/backgroundAssets/gameCenter); `asc iap` and `asc versions` have no attach-IAP. So there is no first-party `asc` subcommand — path 1 (raw API) or 2 (browser).
  - This is a SUBMISSION-time action (it creates/feeds a review submission), so it only matters at M17 with a build present — not a standalone "attach now" step.
